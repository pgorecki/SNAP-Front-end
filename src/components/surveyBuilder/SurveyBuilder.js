...
import {
  ...
  findItemParent,
  iterateItems,
} from 'pages/surveys/computations';
...
function isAssociatedToNonExistingQuestion(item, questions) {
  return (
    item.hmisId && questions.findIndex((q) => q.hmisId === item.hmisId) === -1
  );
}
...
export default class SurveyBuilder extends React.Component {
  ...
  getTreeProps(treeData) {
    ...
    (function getNodeProps(children = []) {
      children.forEach((node) => {
        nodeProps[node.definition.id] = Object.assign(
          nodeProps[node.definition.id] || {},
          {
    ...
    return Object.assign(
      {
        ...
        children: item.items
          ? item.items.map((child) => this.itemToTree(child, nodeProps))
          : undefined,
        className: item.type,
        definition: item,
      },
  ...
  definitionToTree(definition, prevTree) {
    ...
    const tree = definition.items.map((item) =>
      this.itemToTree(item, nodeProps)
    );
    return tree;
  }
  ...
  handleItemClone(itemId) {
    ...
    const parent =
      findItemParent(itemId, this.state.definition) || this.state.definition;
  ...
  handleItemChange(updatedItem, name, previous) {
    ...
    const definition = (function updateItem(root, id, item) {
      ...
      _.each(root.items || [], (child) => {
        updateItem(child, id, item);
      });
      return root;
    ...
    if (missing.length > 0) {
      items.push(
        <p key="item2">
          ...
          {missing.length} questions have missing HMIS Questions:{' '}
          {missing.join(', ')}.
        </p>
      );
    }
