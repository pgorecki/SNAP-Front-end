...
export default class Grid extends Item {
    ...
    switch (item.category) {
      case 'date':
        return (
          <SemanticDatepicker
            disabled={disabled}
            onChange={(event, data) => this.handleChange(event, data.value)}
          />
          // <DatePicker
          //   selected={cellValue ? moment(cellValue).toDate() : ''}
          //   onChange={(value) =>
          //     this.handleChange(r, item.id, value, item.category)
          //   }
      ...
      default:
        return (
          <input
            ...
            value={cellValue || ''}
            onChange={(event) =>
              this.handleChange(r, item.id, event.target.value, item.category)
            }
            disabled={disabled}
          />
        );
  ...
  renderRows(disabled) {
    const { columns, id } = this.props.item;
    const numRows =
      this.props.formState.props[`${id}.rows`] || this.props.item.rows;
    ...
    return (
      <div className="grid item">
        ...
        <table>
          ...
          <tbody>{this.renderRows(disabled)}</tbody>
        </table>
      </div>
    );
  }
